...
export async function GET() {
  try {
    ...
    const recentScans = quickScans.filter(s => new Date(s.createdAt) > thirtyDaysAgo).length
    const reportsGenerated = quickScans.filter(s => s.reportGenerated).length
    const consultationsRequested = quickScans.filter(s => s.wantsConsultation).length
    
    const averageScore = quickScans.length > 0 ? 
      Math.round((quickScans.reduce((sum, scan) => sum + scan.overallScore, 0) / quickScans.length) * 10) / 10 : 0

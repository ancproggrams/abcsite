...
export async function GET() {
  try {
    ...
    const recentConversations = conversations.filter(c => new Date(c.createdAt) > thirtyDaysAgo).length
    const qualifiedConversations = conversations.filter(c => c.status === 'QUALIFIED' || c.status === 'CONVERTED').length
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      const dailyConversations = conversations.filter(c => {
        const createdAt = new Date(c.createdAt)
        return createdAt >= date && createdAt < nextDate
      })
      
      const dailyQualified = dailyConversations.filter(c => c.status === 'QUALIFIED' || c.status === 'CONVERTED')
